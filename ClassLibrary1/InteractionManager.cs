...
namespace GD
{
    class InteractionManager
    {
        ...
        {
            Interactable = false;
            CommandManager.Instance.RegisterCommand("edit", this.EditCommandHandler);
        }
        ...
        private void DevisualizeNPCs()
        {
            lock (this.visualized_npcs)
            {
                foreach (SceneObjectGroup visualized_npc in this.visualized_npcs.Keys)
                {
                    ...
                    //visualized_npc.Scene.DeleteSceneObject(visualized_npc, false);
                    visualized_npc.DeleteGroupFromScene(false);
                }
            }
        }
        ...
        private void DevisualizeGraph()
        {
            lock (this.visualized_nodes)
            {
                foreach (SceneObjectGroup visualized_node in this.visualized_nodes.Keys)
                {
                    ...
                    //visualized_node.Scene.DeleteSceneObject(visualized_node, false);
                    visualized_node.DeleteGroupFromScene(false);
                    GraphNode node;
            ...
            lock (this.visualized_links)
            {
                foreach (SceneObjectGroup visualized_link in this.visualized_links.Keys)
                {
                    ...
                    visualized_link.DeleteGroupFromScene(false);
                }
            }
        }
        ...
        private void AddObjectHook(SceneObjectGroup sog)
        {
            ...
            if (sog.RootPart.Shape.ProfileShape == ProfileShape.Square)
            {
                ...
                NPCharacter npc = new PyNPCharacter("Default", "Name", "default", location, "null");
        ...
        private void SaveNotecard(Dictionary<String, String> content, SceneObjectGroup sog)
        {
            ...
            task_item.BasePermissions = (uint)PermissionMask.All;
            task_item.CurrentPermissions = (uint)PermissionMask.All;
            task_item.EveryonePermissions = (uint)PermissionMask.All;
            task_item.NextPermissions = (uint)PermissionMask.All;
            task_item.GroupID = sop.GroupID;
            task_item.GroupPermissions = (uint)PermissionMask.All;
            task_item.Flags = 0;
        ...
        {
            string first_name, last_name, appearance, script_name;
            ...
            if (parameters.Equals("on") || parameters.Equals("off"))
            {
                ...
                if (turn_on != interaction_manager.Interactable)
                {
                    if (turn_on)
                    {
                        ...
                        InteractionManager.Instance.StartInteraction(character.AgentId);
                    }
                    else
                    {
